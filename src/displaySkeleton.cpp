...
{
  GLUquadricObj *qobj;
  int numbones = m_pSkeleton[skeletonID]->numBonesInSkel(bone);
...
void DisplaySkeleton::DrawBone(Bone *pBone, int skelNum)
{
  ...
  glMultMatrixd((double *)pBone->rot2parent.data());
  /*
...
*/
  ...
  else
  {
    ...
    Eigen::Vector3d pBoneDir;     
    pBoneDir(0) = pBone->dir[0];
    pBoneDir(1) = pBone->dir[1];
    pBoneDir(2) = pBone->dir[2];

    Eigen::Vector3d r_axis = z_dir.cross(pBoneDir);  

    double dot_prod = z_dir.adjoint()*pBoneDir;
...
void DisplaySkeleton::Render(RenderMode renderMode_)
{
  ...
  {  
    ...
    glPushMatrix();
    float sz = 0.2, cx = 320, cy = 240, fx = 700, fy = 700;
    float width = 640;
    float height = 480;
    double tl[3];
    m_pSkeleton[0]->GetRootPosGlobal(tl);
    ...
    tl[2] = tl[2]+ 1.0f;
    //glTranslatef(float(tl[0]), float( tl[1] ), float(tl[2]+ 1.0f));
    float rotCamera[4][4];		
    ...
    for(int ii=0;ii<3;ii++){
      rotCamera[3][ii]=tl[ii];
    }
    glMultMatrixf((GLfloat*)rotCamera);    
